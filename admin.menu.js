...
/* ===== stability patch: group required + 2slot accordion + move_type alias ===== */
function getBaseMenuGroupCatalog(){
  return Array.isArray(getAdminResolvedGroupCatalog()) && getAdminResolvedGroupCatalog().length
    ? getAdminResolvedGroupCatalog()
    : [
...
function _normalizeAliasedGroupForAdmin_(item){
  const key = String(item && item.key || '').trim();
  const group = String(item && item.menu_group || '').trim();
  if (/^MOVE_/i.test(key)) return 'move_type';
  if (group === 'auto_set') return 'auto_set';
  return group || 'custom';
}
...
function adminNormalizeMenuRows(){
  return (adminMenuMaster || []).map((item, idx) => {
    const clone = cloneMenuObject(item || {});
    clone.menu_group = _normalizeAliasedGroupForAdmin_(clone);
    clone.key = makeMenuInternalKey(clone, idx);
    clone.key_jp = String(clone.key_jp || '');
    clone.label = String(clone.label || '');
    clone.price = Number(clone.price || 0);
...
function getMenuItemsByGroup(group){
  const target = String(group || '').trim();
  return (adminMenuMaster || [])
    .filter(item => _normalizeAliasedGroupForAdmin_(item) === target)
    .sort((a, b) => Number(a.sort_order || 9999) - Number(b.sort_order || 9999));
}
...
function buildMenuAutoApplyOptions(selectedGroup, selectedKey){
  ...
  let keyCandidates = [];
  if (selectedGroup) keyCandidates = getMenuItemsByGroup(selectedGroup);
...
function _ensureAutoApplyOpenStore_(){
  if (!window.__menuAutoApplyOpenState) window.__menuAutoApplyOpenState = {};
  return window.__menuAutoApplyOpenState;
}
function _isAutoApplyOpen_(item){
  const key = String(item && item.key || '');
  const state = _ensureAutoApplyOpenStore_();
  if (state[key] !== undefined) return !!state[key];
  return !!(item && (item.auto_apply_group || item.auto_apply_key || item.auto_apply_group_2 || item.auto_apply_key_2));
}
function _setAutoApplyOpen_(itemKey, isOpen){
  const state = _ensureAutoApplyOpenStore_();
  state[String(itemKey || '')] = !!isOpen;
}
...
function renderMenuItemCard(item, groupItems){
  ...
  const autoOpen = _isAutoApplyOpen_(item);
      ...
      <div class="menu-item-top">
        ...
        <div class="flex-1">
          <div class="menu-item-main">
            ...
            <div class="form-group">
              <label class="form-label">現在グループ</label>
              <input type="text" value="${escapeHtml(getGroupLabelByKey(_normalizeAliasedGroupForAdmin_(item) || 'custom'))}" disabled>
            </div>
          </div>
          ...
          <div class="mt-4 rounded-2xl border border-dashed border-sky-200 bg-slate-50/80 overflow-hidden">
            <button class="w-full text-left px-4 py-3 font-bold flex items-center justify-between" data-action="toggleAutoApply" data-key="${escapeHtml(item.key || '')}" type="button">
              <span>⚙ 自動セット設定（${autoCount ? `${autoCount}件設定中` : '未設定'}）</span>
              <span class="text-sm text-slate-500">${autoOpen ? 'クリックで閉じる' : 'クリックで開く'}</span>
            </button>
            <div class="px-4 pb-4 ${autoOpen ? '' : 'hidden'}" id="autoApplyWrap_${escapeHtml(item.key || '')}">
              <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-4 pt-3 border-t border-dashed border-sky-200">
                <div class="form-group">
    ...
    <div class="menu-group-card" data-menu-group="${escapeHtml(group)}">
      <div class="menu-group-card-header" data-action="toggleMenuGroup" data-group="${escapeHtml(group)}">
        ...
        <div class="flex items-center gap-2">
          ...
          <button class="move-btn" data-action="groupDown" data-group="${escapeHtml(group)}" type="button" ${isFixedMenuGroup(group) || groupIndex < 1 || groupIndex >= order.length - 1 ? 'disabled' : ''}>↓</button>
          <button class="move-btn" data-action="menuAddInGroup" data-group="${escapeHtml(group)}" type="button">＋</button>
          ${canDelete ? `<button class="move-btn" data-action="groupDelete" data-group="${escapeHtml(group)}" type="button">🗑</button>` : ''}
          <div class="menu-group-card-toggle" data-menu-group-toggle="${escapeHtml(group)}">${open ? '−' : '＋'}</div>
        </div>
      </div>
      ...
      <div class="menu-group-card-body ${open ? '' : 'collapsed'}" id="menuGroupBody_${escapeHtml(group)}">
        <div class="mb-4 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          <div class="form-group">
            <label class="form-label">グループ必須 / 任意</label>
            <select data-group-field="required_flag" data-group="${escapeHtml(group)}" ${['price','custom','auto_set'].includes(String(group || '')) ? 'disabled' : ''}>
        ...
        <div>
          ${items.length ? items.map(item => renderMenuItemCard(item, items)).join('') : `<div class="text-sm text-slate-500 font-bold py-3">まだ項目がありません</div>`}
        </div>
...
function buildSaveMenuPayload(){
  ...
  return adminMenuMaster.map((item, idx) => {
    const label = String(item.label || '').trim();
    let group = _normalizeAliasedGroupForAdmin_(item);
    const key = String(item.key || '').trim() || makeMenuInternalKey(item, idx);
...
function bindMenuEvents(){
  ...
  wrap.addEventListener('click', (e)=>{
    ...
    if (action === 'groupDelete'){
      ...
      adminConfig.menu_group_catalog_json = JSON.stringify(catalog);
      const order = getEffectiveMenuGroupOrder().filter(k => String(k || '') !== String(group || ''));
      adminConfig.menu_group_order_json = JSON.stringify(order);
      const visibility = cloneMenuObject(getStoredMenuGroupVisibility());
    ...
    if (action === 'toggleAutoApply'){
      const current = _isAutoApplyOpen_({ key: key });
      _setAutoApplyOpen_(key, !current);
      renderMenuAdminList();
      return;
    }
    if (action === 'groupUp'){
