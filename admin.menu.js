...
function adminNormalizeMenuRows(){
  return (adminMenuMaster || []).map((item, idx) => {
    ...
    if (String(clone.menu_group || '') === 'custom' && /^MOVE_/.test(String(clone.key || ''))){
      clone.menu_group = 'move_type';
    }
    if (String(clone.menu_group || '') === 'equipment' && String(clone.key || '') === 'EQUIP_STRETCHER_STAFF2'){
      clone.menu_group = 'auto_set';
    }
