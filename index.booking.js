...
/* ===== reservation save safety restore patch start ===== */
submitBooking = async function(e){
  e.preventDefault();
  ...
  const usageType = document.getElementById('usageType') ? document.getElementById('usageType').value : '';
  ...
  const pickupLocation = document.getElementById('pickupLocation') ? document.getElementById('pickupLocation').value.trim() : '';
  ...
  const assistanceType = document.getElementById('assistanceType') ? document.getElementById('assistanceType').value : '';
  const stairAssistance = document.getElementById('stairAssistance') ? document.getElementById('stairAssistance').value : '';
  const equipmentRental = document.getElementById('equipmentRental') ? document.getElementById('equipmentRental').value : '';
  const roundTrip = document.getElementById('roundTrip') ? document.getElementById('roundTrip').value : '';
  ...
  const slotDateStr = ymdLocal(selectedSlot.date);
  const reservationDatetime = `${slotDateStr} ${String(selectedSlot.hour).padStart(2,'0')}:${String(selectedSlot.minute).padStart(2,'0')}`;
  ...
  const reservation = {
    ...
    stair_assistance: stairAssistance,
    equipment_rental: equipmentRental,
    stretcher_two_staff: stretcherTwoStaff,
    ...
    'お名前': customerName,
    '連絡先': phoneNumber,
    'お伺い場所': pickupLocation,
    'お伺い先': pickupLocation,
    '送迎先': destination || '',
    '移動方法': moveType,
    '介助内容': assistanceType,
    '階段介助': stairAssistance,
    '機材': equipmentRental,
    '2名体制': stretcherTwoStaff,
    '往復': roundTrip,
    '備考': notes || '',
    '料金': Number(total || 0),
    'ステータス': '未対応',
    '予約日': slotDateStr,
    '予約時': selectedSlot.hour,
    '分': selectedSlot.minute,
    '表示': true
  };
...
/* ===== reservation save safety restore patch end ===== */
